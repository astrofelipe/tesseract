...
dset   = output.create_dataset('FFIs', (nx, ny, nfiles), dtype='float64', chunks=(32, 32, nfiles), compression='lzf')
derr   = output.create_dataset('errs', (nx, ny, nfiles), dtype='float64', chunks=(32, 32, nfiles), compression='lzf')
...
for i,f in enumerate(tqdm(files)):
    if i % nprocs == rank:
        ...
        dset[:,:,i] = np.expand_dims(dat1, axis=0)
        derr[:,:,i] = np.expand_dims(dat2, axis=0)
