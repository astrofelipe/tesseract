...
dset   = output.create_dataset('FFIs', (nfiles, nx, ny), dtype='float64', chunks=(64, 32, 32), compression='gzip')
derr   = output.create_dataset('errs', (nfiles, nx, ny), dtype='float64', chunks=(64, 32, 32), compression='gzip')
...
for i,f in enumerate(tqdm(files)):
    if i % nprocs == rank:
        ...
        #with dset.collective: dset[i:i+1] = np.expand_dims(dat1, axis=0)
        #with derr.collective: derr[i:i+1] = np.expand_dims(dat2, axis=0)
        #with table.collective: table[:,i] = make_table(f)
